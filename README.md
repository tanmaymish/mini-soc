...
| Layer | Host | How |
|-------|------|-----|
| **Dashboard** (React) | **GitHub Pages** | Auto-deploys on every push to `master` via `.github/workflows/deploy-pages.yml`. Ships an **interactive demo** laid out as a real security-operations console — a left-hand nav rail (Overview · Alerts · Incidents · Detections · Automation · Simulation), a top command bar with global alert search, and live status. Click (or press `1`–`0`) to launch attacks, or flip on **Live Fire** (`L`) for a continuous randomized attack stream, and watch live charts, a streaming log console, and auto-containment react in real time. Work the queue like an analyst: search/filter the alert feed, acknowledge/resolve/dismiss alerts, push manual IP blocks into the SOAR feed, and watch your MTTA/MTTR on the scoreboard. Or play **SOC Defense** (`D`) — a real-time game where attacks stream in with breach countdowns and you race to contain them before they drain the SOC's integrity, scoring combos across escalating waves for an analyst rank. No backend required. Live at **[tanmaymish.github.io/mini-soc](https://tanmaymish.github.io/mini-soc/)**. |
